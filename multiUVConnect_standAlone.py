...
class multiUVConnect:
    ...
    def checkMaterial(self, *args):
        ...
        fileNodes = self.findFileTxNode(mtl)
    ...
    def updateTextField(self, field, objType='', multiple=False, *args):
        ...
        if multiple:
            ...
            if len(sele) == 1:
                ...
                cmds.textField(field, e=True, tx=self.cleanListForPrint(sele))
        else:
            if len(sele) == 1:
                if objType:
    ...
    def resizeScroll(self, scroll, min=100):
        allItems = cmds.textScrollList(scroll, q=True, allItems=True)
        if not allItems:
            cmds.textScrollList(scroll, e=True, h=min)
            return
        if len(allItems) > 16:
    ...
    def cleanListForPrint(self, inList):
        printStr = "['%s'" % inList[0]
