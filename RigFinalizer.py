...
class RigFinalizer:
    def __init__(self, par=''):
        ...
        # todo add show joints check
        # todo ignore "transform" (outside of main)
    ...
    def hierarchyCheck(self, field):
        hGrpObjs = pm.listRelatives('high_grp', ad=True, type='transform')  # todo high_grp checks
        constraints = pm.listRelatives('high_grp', ad=True, type='constraint')

        errors = []
        self.resetProgBar2(hGrpObjs, lenList=True)
        for obj in hGrpObjs:
            grp = False
            okObj = False
            added = False
        ...
        for obj in constraints:
            par = obj.getParent()
            shp = pm.listRelatives(par, s=True)
            if shp:
    ...
    def deformCheck(self):
        ...
        errors = []
        notGood = []
        self.resetProgBar2(deformObjs, lenList=True)
    ...
    def transformMeshesCheck(self, meshes, needCns, needCns_attrs):
        hasDeformers = []
        missingCns = []
        self.resetProgBar2(meshes, lenList=True)
        for obj in meshes:
            shps = pm.listRelatives(obj, s=True)
            for shp in shps:
                connections = pm.listConnections(shp, d=False, s=True)
    ...
    def transformObjsFilter(self, transformObjs, constraints):
        ...
        self.resetProgBar2(transformObjs, lenList=True)
    ...
    def checkConstrainted(self, obj, needCns_attrs, transformGrp=False):
        ...
        cnsAttrs = cmds.listConnections(objName, s=True, d=False, type='constraint', p=True)
        if cnsAttrs:
            self.resetProgBar2(cnsAttrs, lenList=True)
            for transAt in needCns_attrs:
                i = 0  # bad count
                ii = 0  # good count
                notFound = []
    ...
    def transformCheck(self):
        ...
        transformObjs = set(pm.listRelatives(transform, ad=True, type='transform'))
        constraints = pm.listRelatives(transform, ad=True, type='constraint')
        transformObjs = list(transformObjs.difference(set(constraints)))
    ...
    def getWorldChilds(self, assetType):  # todo ignore references
    ...
    def ref_hGrpOverride(self, *args):
        ...
        obj_dispType = []
        obj_highGrp = False
        self.resetProgBar2(hGrp_ad, lenList=True)
        for obj in hGrp_ad:
    ...
    def ref_hGrpVals(self, *args):
        field = 'hGrpVals'
        self.widgets[field + '_items'] = []
        if not pm.objExists('high_grp'):  # todo high_grp check
            self.addLogHeader(field)
            self.addLog('"high_grp" error')
            self.widgets[field + '_items'] = []
        ...
        hGrp_ad.append('main')  # todo main search
        objs_to_fix = []
        self.resetProgBar2(hGrp_ad, lenList=True)
        for obj in hGrp_ad:
            for atr in 'trs':
    ...
    def ref_animKeys(self):
        ...
        self.resetProgBar2(main_ad, lenList=True)
        obj_keys_list = []
        for obj in main_ad:
    ...
    def ref_ctlVals(self):
        ...
        ctls = pm.ls('*_ctl')
        withVals = []
        self.resetProgBar2(ctls, lenList=True)
        for ctl in ctls:
    ...
    def ref_mainCns(self):
        ...
        self.resetProgBar2(cnsLs, lenList=True)
        for cns in cnsLs:
            shp = cns.getShape()
            if shp:
    ...
    def ref_cAttrs(self):  # todo check if matches shading file
        field = 'cAttrs'

        # c_attrs check
        errors = []
        if not pm.objExists('high_grp'):  # todo high_grp search
            self.updateTextFramesStat(field, red=True, fieldNote='No "high_grp" found')
            return
        ...
        self.resetProgBar2(allAttrs, True)
    ...
    def ref_naming(self):
        ...
        if not pm.objExists('main'):  # todo main search
            # todo high_grp check
        ...
        cards = pm.ls('*_card')
        self.resetProgBar2(ad, lenList=True)
        for obj in ad:
            obj = obj.name()
            guideItem = False
            hGrpItem = False
        ...
        if doubles_hGrp or doubles_guides or doubles_general or cards:
            ...
            if doubles_guides:
                semi_c = {True: '\n ** use "Select Win" or un-check "ignore guide" to print **', False: ':'}
                ignore_guide = pm.checkBox(self.widgets['guideNaming'], q=True, v=True)
                self.addSpace(space)
                self.addLog('names clashing for objects under a guide%s' % semi_c[ignore_guide])
                space = True
                if not ignore_guide:
        ...
        if ref_nSpaces:
            ...
            if set_or_prop:
                if not pm.objExists('assembly'):
                    asm_errors.append('References found in scene - "assembly" node is missing')
                else:
                    if 'network' not in pm.PyNode('assembly').type():
                        asm_errors.append('"assembly" node is of wrong type')
            else:
    ...
    def ref_references(self):
        ...
        if set_or_prop:  # check for unloaded
            for ref in ref_list:
                name = pm.referenceQuery(ref, rfn=True)
    ...
    def ref_ngSkinTools(self):
        ...
        if ngNodes:
            ...
            self.updateTextFramesStat(field, yellow=True, fieldNote='ngSkinTools nodes found')
        else:
            self.widgets[field + '_items'] = []
    ...
    def resetProgBar2(self, barLength, lenList=False):
        if lenList:
            if isinstance(barLength, list) or isinstance(barLength, tuple):
                barLength = len(barLength)
            else:
                barLength = 1
    ...
    def set_or_prop(self):
        assetType = self.getAssetType()
        set_or_prop = False
        if 'set' in assetType or 'prop' in assetType:
            set_or_prop = True
        return set_or_prop
    ...
    def layoutAssetDetails(self, mainLay):
        ...
        pm.columnLayout(rowSpacing=1, p=lay)  # , columnOffset2=[0, 5], adjustableColumn=2, p=mainLay)
        self.widgets['clearLog'] = pm.checkBox(l='Clear Log', v=1)
        self.widgets['guideNaming'] = pm.checkBox(l='ignore guide', v=1)
