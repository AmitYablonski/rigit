...
class MassTools:
    ...
    def massWin(self):
        ...
        cmds.button(l="Filter Mesh\nSelection\nHierarchy", h=h, w=w, bgc=self.green2,
                    ...
                    ann='Filters selection only to transforms that have a shape in hierarchy')
        cmds.button(l="Mass\nAttributes\nTransfer", h=h, w=w, bgc=self.purple1, c=self.MassAttrTransfer)
        cmds.button(l="Filter Jnts\nSelection", h=h, w=w, bgc=self.orange2, c=gmt.filterJointSelection,
                    ann='Filters selection only to joints')
        cmds.button(l="Filter Jnts\nSelection\nHierarchy", h=h, w=w, bgc=self.orange2,
                    c=partial(gmt.filterJointSelection, True),
                    ann='Filters selection only to joints in hierarchy')
