...
class GroupIt_QT(QtWidgets.QMainWindow):
    ...
    def groupItWin(self):
        ...
        # self.layout = QtWidgets.QFormLayout(self.mainWidget)
    ...
    def groupItWin_populate(self):
        # todo learn how to use fonts QTGui.QFont()?

        # addLayout(layout)
        # removeLayout(lay)
        # QtWidgets.addStretch() # adds a spaces that pushes elemrnts around it
        # addSpace(size) # adds a specific amount of space
        ...
        button.clicked.connect(self.groupIt)  # todo associate correct command
    ...
    def groupItFix(self, *args):  # todo this...
        op = self.suffix_button.isChecked()
        grpText = self.textLine.text()
        #for button, preset in self.presetButtons:
        '''
