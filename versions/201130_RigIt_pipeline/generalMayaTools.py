...
def transferWeights(*args):
    ...
    print(mainVtx)  # todo remove prints?
    print(transVal)
    print(verts)
...
def rotateToJntOrient(toRotate=False, *args):
    ...
    for item in selection:
        if pm.objectType(item) == 'joint':
            ...
            print('skipping - "%s" is not a joint')
...
def selectBsp(*args):
    ...
    for obj in sele:
        ...
        if not bShapes:
            print(' // no Blend Shape node found on "%s"' % obj)
        else:
            for bsp in bShapes:
                allBsps.append(bsp)
    pm.select(allBsps)
...
def createBsp(add=False, *args):
    ...
    bShape =''
    if not add:
        bShape = pm.blendShape(bsps, obj, n=obj + '_blendShape', foc=True)
    else:
        bShapes = findBlendShape(obj)
        if not bShapes:
            print(' // no Blend Shape node found on "%s"' % obj)
...
def instanceToObject(*args):
    pm.scriptEditorInfo(se=1, sw=1, si=1)
    instaList = pm.ls(sl=True)
    for ins in instaList:
        print('// # checking if "%s" is instance : ' % ins)
        mel.eval('ConvertInstanceToObject')
    pm.scriptEditorInfo(se=0, sw=0, si=0)
...
def filterJointSelection(hierarchy=False, *args):
    selection = pm.selected()
    if hierarchy:
        selection = updateToHierarchy(selection)
    jnts = pm.ls(selection, type='joint')
    pm.select(jnts)
...
def compare_vtx_position(source, target):
    vCount = pm.polyEvaluate(source, v=True)
    offset_vtx = []
    for i in range(0, vCount):
        vtx_source = pm.xform(str(source + '.vtx[%s]' % str(i)), q=True, t=True, a=True)
        vtx_target = pm.xform(str(target + '.vtx[%s]' % str(i)), q=True, t=True, a=True)
        for j in range(0, 2):
            if round(vtx_source[j], 3) != round(vtx_target[j], 3):
                offset_vtx.append(target + '.vtx[' + str(i) + ']')
    # todo print / select offset vtx


def is_vtx_count_equal(obj1, obj2):
    obj1_vtx = pm.polyEvaluate(obj1, v=True)
    obj2_vtx = pm.polyEvaluate(obj2, v=True)
    if obj1_vtx == obj2_vtx:
        return True
    else:
        return False


def is_obj_identical(source, target):
def is_obj_identical(source, target):
    is_identical = False
    if is_vtx_count_equal(source, target):
        dup = ''
        try:  # todo finish this check up
            dup = pm.duplicate(target, )[0]
            bShape = pm.blendShape(source, dup, n="bsp_temp")[0]
            if '|' in source:
                source = source.rpartition('|')[2]
            bShape.setAttr(source, 1)
...
def getMaterials(item, window=''):
    materials = []
    if isinstance(item, (list, tuple)):
        for obj in item:
            materials += getMaterials_fromObj(obj, window)
    else:
        materials = getMaterials_fromObj(item, window)
    return materials


def getMaterials_fromObj(item, window=''):
    materials = []
    if not pm.objExists(item):
        error = 'gmt.getMaterials - object not found: "%s"' % item
        if window:
            window.printFeedback(error)
        else:
            print(' // Error !! ' + error)
        return materials
    ...
    for con in conn:
        if con.classification()[0] == 'shadingEngine':
            ...
            if 'shader' in con.classification()[0]:
                if con not in materials:
                    materials.append(con)
    return materials


def get_faceColor(item):
    '''
    retruns a dictionary
    :param item: a transform obj with
    :return: dict
    '''
    materials = getMaterials(item)
    relevant_faces = {}
    if not materials:
    ...
    for mat in materials:
        ...
        sele = pm.ls(sl=True, fl=True)
        objs_mtl = []
        for sel in sele:
            shp_name = item.getShape().name()
            if shp_name in sel.name():
                objs_mtl.append(sel)
        relevant_faces[mat] = objs_mtl
    return relevant_faces


def get_vtx_position_face(target):
    '''

    :param target: object with mesh
    :return: dictionary
    dictionary_exmple_1 = {face_num: [[x_pos, y_pos, z_pos], [x_pos, y_pos, z_pos], [x_pos, y_pos, z_pos]]}
    dictionary_exmple_2 = {0: [[0,0,0], [0,0,1], [0,1,0]], # triangle face with 3 verts
                           1: [[1,0,0], [1,0,1], [1,1,0], [1,1,0]]} # square face with 4 verts
    '''
    target_faces = pm.polyEvaluate(target, f=True)
    all_faces_vtx = {}
    for i in range(target_faces):
        vtx_pos = pm.xform(str(target + '.f[%s]' % str(i)), q=True, t=True, ws=True)
        face_vtx = []
        ii = 0
        temp = []
        for val in vtx_pos:
            ...
            ii += 1
            if ii > 2:
                face_vtx.append(temp)
                ii = 0
                temp = []
        all_faces_vtx[i] = face_vtx
    return all_faces_vtx


'''
# this example creates locators on every vertex
all_faces_vtx = get_vtx_position_face(target)
grp = pm.group(n='%s_locators_grp' % target, em=True, w=True)
for face_num in all_faces_vtx:
    face_vtx = all_faces_vtx[face_num]
    for pos in face_vtx:
        loc = pm.spaceLocator(n='%s_face_%s_01' % ( target, str(format(face_num, '0' + str(2))) ))
        pm.parent(loc, grp)
        pm.xform(loc, t=pos, ws=True)
'''


def get_face_center_pos(target):
    average_dict = {}
    all_faces_vtx = get_vtx_position_face(target)
    for face_num in all_faces_vtx:
        face_vtx = all_faces_vtx[face_num]
        ...
        for pos in face_vtx:
            for i in range(0, 3):
                average[i] = average[i] + pos[i]
        for i in range(0, 3):
            average[i] = average[i] / len(face_vtx)
        average_dict[face_num] = average
    return average_dict


'''
# this example creates locators on every face
grp = pm.group(n='%s_locators_grp' % target, em=True, w=True)
average_dict = gmt.get_face_center_pos(target)
for face_num in average_dict:
    pos = average_dict[face_num]
    loc = pm.spaceLocator(n='%s_face_%s' % ( target, str(format(face_num, '0' + str(2))) ))
    pm.parent(loc, grp)
    pm.xform(loc, t=pos, ws=True)
'''


def get_closest_vector(vector, traget_list):
    return
...
def copy_material(*args):
    ...
    if not mtl_list:
        print(' // copy_material: No material found on %s' % source)
        return
    if len(mtl_list) == 1:
        pm.select(targets)
        ...
        for target in targets:
            print(' // copy_material: Copying multiple materials from %s to %s' % (source, target))
            is_identical = is_obj_identical(source, target)
            if is_identical:
                for mtl in mtl_list:
                ...
                source_faces = get_face_center_pos(source)
                target_faces = get_face_center_pos(target)
                closest_assos = {}
                for face_num in target_faces:
                    ...
                    closest_dist = get_distance(current_face_pos, source_faces[0])
                    for i in range(0, len(source_faces)):
                        temp = get_distance(current_face_pos, source_faces[i])
                    ...
                    if not shader:
                        shader = 'lambert1'
                        print(' // copy_material: Error finding a shader on "%s"  -  assigning lambert1 instead' % face)
                    else:
                        shader = shader[0]
...
def find_double_names(high_grp=False):
    init_sel = pm.selected()
    ...
    if high_grp:
        if not pm.objExists('high_grp'):
            return return_dict
        all_objs = pm.listRelatives('high_grp', ad=True)
    else:
        pm.select(all=True)
        all_objs = pm.listRelatives(pm.selected(), ad=True)
    for obj in all_objs:
        obj_name = obj.name()
        if '|' in obj_name:
            obj_doubles = pm.ls(obj_name.rpartition('|')[2])
            if len(obj_doubles) < 2:
                continue
            if high_grp:
                under_hGrp = []
                for item in obj_doubles:
                    if isUnderPar(item, 'high_grp'):
                        under_hGrp.append(item)
                if under_hGrp > 1:
                    obj_doubles = under_hGrp
                else:
                    obj_doubles = []
            for item in obj_doubles:
                item_name = item.name()
                if item_name not in doubles:
                    print item_name
                    doubles.append(str(item_name))
    if doubles:
        return_dict["result"] = True
        return_dict["nodes"] = doubles
    if not init_sel:
        pm.select(cl=True)
    else:
        pm.select(init_sel)
...
def get_transform_vals():
    ...
    for sel in sele:
        ...
        for at in "tr":
            for tr in "xyz":
                attr = sel.attr(at + tr)
                val = float(round(attr.get(), 3))
                if val:
                    temp.append([at + tr, val])
        if temp:
            beret_hair_vals[str(sel.name())] = temp
    return ctl_vals


def mirrorUV(obj):
    pm.select(obj)
    allUvs = pm.polyListComponentConversion(tuv=True)
    mel.eval("changeSelectMode -component; selectType -ocm -alc false; selectType -alc false; selectType -puv true; selectType -suv true;")
    pm.select(allUvs)
    pm.setAttr(obj + ".uvPivot", [0.5, 0.5])
    pm.polyEditUV(pu=0.5, pv=0.5, su=-1, sv=1)

def mirrorUV_manual(obj):
    pm.select(obj)
    allUvs = pm.polyListComponentConversion(tuv=True)
    pm.select(allUvs)
    allUV = cmds.ls(sl=True, fl=True)
    for uv in allUV:
        pm.select(uv)
        uv_pos = pm.polyEditUV(uv, query=True)
        pm.polyEditUV(uv, relative=False, uValue=(1-uv_pos[0]))
    pm.select(sele)
