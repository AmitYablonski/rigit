...
class RigFinalizer:
    ...
    def refHierarchy(self):
        ...
        # todo fix 2nd progBar for this method
    ...
    def doChecks(self, field, ignoreChBox=False, *args):
        # todo write this code smarter (like cases)
        chBox = pm.checkBox(self.widgets[field + 'chBox'], q=True, v=True)
        if chBox and not ignoreChBox:
            return
        ...
        if 'hGrpOverride' in field:
            self.refHighGrpOverride()

        if 'hGrpVals' in field:  # values on geos
            self.refHighGrpValues()

        if 'animKeys' in field:
            self.refAnimKeys()

        if 'ctlVals' in field:  # transform values on ctls check
            self.refCtlVals()
            # todo fix: zero out ctls and delete keyframes?

        if 'mainCns' in field:  # CNS for main ctrls check
            self.refCtlCns()

        if 'lambert1' in field:
            self.refLambert1()
        ...
        if 'nameSpaces' in field:
            self.refNameSpaces()

        if 'assembly' in field:
            self.refAssembly()
    ...
    def stepsLayBase(self, parLay, header, field, fix, select, delete=False): # todo remove fieldTx
        ...
        self.widgets[field + 'chBox'] = pm.checkBox(l='Skip', p=lay)
        pm.button(l='check', c=partial(self.doChecks, field, True), p=lay)
    ...
    def populate_steps(self, parLay):
        ...
        pm.button(buttons_dict['references'], e=True, l='Reload', c=self.fixUnloadedRefs)
        pm.button(buttons_dict2['references'], e=True, l='Remove', c=self.removeUnloadedRefs)
    ...
    def winBase(self, name, title, par):
        ...
        self.widgets[mainLay] = cmds.paneLayout(configuration='vertical2', staticWidthPane=1)
