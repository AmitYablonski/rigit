...
class Varianter:
    ...
    def varianterLayout(self, parent='None'):
        ...
        cmds.menuItem(label='Explore to base Character',
                      c=partial(self.openFolder, 'base')) # todo make this open the main char file
    ...
    def resetRunnerMaker(self):
        ...
        self.varScriptsLayoutOld(self.widgets["runnerMakerLayoutParent"])
        self.enableRunnerMaker(False)
    ...
    def varScriptsTopLayout(self, parLay):
        ...
        self.varScriptsLayoutOld(mainLay)
        return mainLay
    ...
    def createNewRunner(self, *args):
        ...
        if not cmds.file(filePath, query=True, exists=True):
            self.scriptIt.printFeedback("Path not found. Check if asset's folders exist, if not, populate through pipeline.")
            return
    ...
    def updateHighObjs(self):
        ...
        try:  # todo once gmt is updated, remove the try/except
            ...
            print
            "//WARNING! general maya tools isn't updated"
    ...
    def initializeVariants(self):
        ...
        for char in sortList:
            ...
            # todo create and populate characterss to drop down scroll
    ...
    def varSelected(self, *args):
        ...
        if var:
            ...
            self.scriptIt.printFeedback('variant selection failed')
            self.var = ''
            return
        filePath = self.pathDict[self.char][self.var] + '\customSteps'
    ...
    def fileToScript(self, *args):
        ...
        script = ''
        if not pyFile:
            self.scriptIt.printFeedback(' failed selecting file')
            return
        script = rim.readFile(filePath + '\\' + pyFile[0])
        if not script:
            # todo make it self.scriptIt.printFeedback(...)
    ...
    def openFolder(self, folder='', *args):
        if self.char and self.var:
            ...
            print
            ' // Not enough items selected to open explorer! please select a character and a variant'
            return
