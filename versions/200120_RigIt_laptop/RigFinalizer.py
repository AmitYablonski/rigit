...
class RigFinalizer:
    ...
    def deformCheck(self):
        # todo deformCheck() - currently doing in maya
        '''
        deform = pm.PyNode('high_grp|deform')
        shps = pm.listRelatives(deform, ad=True, type='mesh')
        ignore = ['tweak', 'groupParts', 'objectSet', 'groupId', 'shadingEngine', 'nodeGraphEditorInfo', 'plusMinusAverage', 'transform']
        notGood = []
        for shp in shps:
            ok = False
            for obj in pm.listConnections(shp, d=False, s=True):
                objType = obj.type()
                if objType not in ignore:
                    print objType
                    ok = True
            if not ok:
                notGood.append(shp.getParent())
        print notGood
        '''
        return
            ...
            if doubles_guides:
                ...
                for obj in doubles_guides:
                    ...
                    for obj2 in doubles_guides[obj]:
                        self.addLog('"%s"' % obj2, space=2)
            # todo check each item in doubles_general to verify there's a double
            if doubles_general:
                self.addSpace(space)
    ...
    def isTooFarFromParent_orig(self, obj, steps, printLog=False, amt=None, objs=[]):
        if not amt:
            amt = steps
        ch = pm.listRelatives(obj, c=True, type='transform')
        if ch:
            if not steps:
                ...
                if printLog:
                    ...
                    ch2 = pm.listRelatives(obj, c=True, ad=True, type='transform')
                    for c in ch2:
                        if c.name() not in objs:
                            shps = pm.listRelatives(c, type=['mesh'])
                            if shps:
                                objs.append(c.name())
                                if printLog:
                                    self.addLog(str(c.name()))
                else:
                    return True
            for c in ch:
                temp = self.isTooFarFromParent(c, steps - 1, printLog, amt, objs)
                if not printLog and temp:
                    return True
