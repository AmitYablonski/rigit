...
def getFilesFromPath(path, typE):
    ...
    try:
        if isinstance(typE, list) or isinstance(typE, tuple):
            ...
            return
        if len(files) == 0:
            print('getFilesFromPath    =>    No files found in path: ' + path
                  +'\ngetFilesFromPath    =>    Type/s searched for: %s' % typE)
    except:
        print ' // Failed to get files from path: %s' % path
        print " // Possibly path doesn't exists"
    return files
