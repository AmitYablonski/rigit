...
def printVal(pport):
    print
    pm.palettePort(pport, query=True, rgb=True)
...
def getChildrenFromFile(filePath, objParent):
    ...
    if cmds.file(filePath, query=True, exists=True):
        ...
        print
        ' // failed reading file: %s' % filePath
    return parList
...
def setDrivenKey(driver, driven, driverVal, drivenVal):
    print
    ' -> doing sdk driver="%s", driven="%s", driverVal=%s, drivenVal=%s' % (driver, driven, driverVal, drivenVal)
