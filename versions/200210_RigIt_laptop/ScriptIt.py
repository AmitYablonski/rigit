...
import SI_VariantMaker as VarMaker
...
class ScriptIt:
    ...
    def scriptIt(self, scriptItPath):
        ...
        self.widgets["window"] = cmds.window("scriptIt_window", title="Script It!",
                                             sizeable=1)  # , rtf=True)
        ...
        # todo figure how to create a multi layer tab layout - concept: 2 layouts, sec and main. when a tab is selected it re-parents the active tab to the main layout
        ...
        cmds.showWindow()
        cmds.window(self.widgets["window"], edit=True, h=400)  # , w=425
        self.defaultFeedback()

        # todo pass a consistent width and hight to all tabs
        VarMaker.VarMaker(parent, self)
        Varianter.Varianter(parent, self)
        ColorIt.ColorIt(parent, scriptIt=True)
        #
        ...
        #'''
        LiteWrap.LiteWrap(parent, scriptIt=True)
    ...
    def addScript(self, add, *args):
        ...
        if tab == "VarMaker":
            script = VarMaker.VarMaker.getScript()
            cmds.scrollField(self.widgets["scriptItField"], e=True, text="")
        if tab == "Varianter":
            return
        if script:
    ...
    def greenFeedback(self, text):
        self.printFeedback(text, green=True)

    def orangeFeedback(self, text):
        self.printFeedback(text, orange=True)

    def printFeedback(self, text, orange=False, green=False):
