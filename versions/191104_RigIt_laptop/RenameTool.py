...
class RenameTool:
    ...
    def renameToolWin(self):
        ...
        cmds.menuItem(divider=True, l="low/upper case")
        cmds.menuItem(label='Selection to upper case', c=partial(self.caseUpdater, True))
        cmds.menuItem(label='Selection to lower case', c=partial(self.caseUpdater, False))
        ...
        self.widgets["case_Layout"] = cmds.rowColumnLayout(numberOfColumns=2, cs=[2, 4],
                                                           p=self.widgets["extras_mainLayout"])
        cmds.button(l='Selection\nTo Upper Case', w=148, h=40, c=partial(self.caseUpdater, True))
        cmds.button(l='Selection\nTo Lower Case', w=148, h=40, c=partial(self.caseUpdater, False))
        cmds.separator(h=7, w=300, p=self.widgets["extras_mainLayout"])
        cmds.separator(h=5, w=300, p=self.widgets["extras_mainLayout"], bgc=[.2, .2, .2])
    ...
    def addMethod(self, selection, method, line2, *args):
        if method == 1:  # "entireName"
            ...
            for sel in selection:
                self.renameObj(sel, line2 + sel)
        elif method == 3:  # "suffix"
            for sel in selection:
                self.renameObj(sel, sel + line2)
