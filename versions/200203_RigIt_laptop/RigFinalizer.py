...
class RigFinalizer:
    ...
    def refHierarchy(self):  # todo hierarchy checkups - other than sets
        # todo add too far objects to selection
        field = 'hierarchy'
    ...
    def transformCheck(self):
        ...
        print ' //// AFTER %s' % needCns_attrs
        for obj in hasDeformers:
            errors.append(self.suffix(obj) + 'Has a deformer')
        for obj in missingCns:
            errors.append(self.suffix(obj) + "isn't fully constrained (trans/rot/scale)")
        for obj in hasChildren:
            errors.append(self.suffix(obj) + "Shouldn't have Constraints/children")
        for obj in emptyGrps:
            errors.append(self.suffix(obj) + 'Empty group')
        for obj in badPivot:
            errors.append(self.suffix(obj) + 'Pivot Not centered')
        for obj in notMeshesShapes:
            errors.append(self.suffix(obj) + 'Shape isn\'t "mesh"')
    ...
    def transformMeshesCheck(self, meshes, needCns, needCns_attrs):
        print 'transformMeshesCheck meshes: %s ' % meshes
        hasDeformers = []
        missingCns = []
        for obj in meshes:
            ...
            if needCns:
                print 'transformMeshesCheck needCns: "%s" %s ' % (obj, needCns)
                temp = []
    ...
    def hasCns(self, obj, needCns_attrs, objCount=0):
        print 'looking on "%s" for %s' % (obj, needCns_attrs)
        # todo fix this
    ...
    def refNaming(self):
        ...
        if doubles_hGrp or doubles_guides or doubles_general or cards:
            ...
            if doubles_general:
                ...
                self.addSpace(True)  # todo <- leave or remove? <-
