...
class CurveRigTool:
    ...
    def getCurveSettings(self):
        upr_crv = cmds.textField(self.widgets['topCrv'], q=True, tx=True)
        lwr_crv = cmds.textField(self.widgets['lowCrv'], q=True, tx=True)
        ...
        if leftAxisOp % 2:
            ...
            leftPositive = -1
        return upr_crv, lwr_crv, leftAxis, leftPositive, jntsAmount

    def executeLips(self, *args):
        # todo make sure this line works:
        upr_crv, lwr_crv, leftAxis, leftPositive, jntsAmount = self.getCurveSettings()
        self.lipsRigOnCrvs(upr_crv, lwr_crv, leftAxis, leftPositive, jntsAmount)
    ...
    def lipsRigOnCrvs(self, upr_crv, lwr_crv, leftAxis, leftPositive, jntsAmount=11, *args):
        ...
        # L
        self.delParCns(["temp_%s_1_jnt" % upr_pref, "temp_%s_1_jnt" % lwr_pref], "echo_MouthCorner_L0_ctl_npo")

        self.delParCns("temp_%s_2_jnt" % upr_pref, "echo_MouthSneer_L0_ctl_npo")
        self.delParCns("temp_%s_2_jnt" % lwr_pref, "echo_MouthSneer_L1_ctl_npo")

        # R
        self.delParCns(["temp_%s_5_jnt" % upr_pref, "temp_%s_5_jnt" % lwr_pref], "echo_MouthCorner_R0_ctl_npo")

        self.delParCns("temp_%s_4_jnt" % upr_pref, "echo_MouthSneer_R0_ctl_npo")
        self.delParCns("temp_%s_4_jnt" % lwr_pref, "echo_MouthSneer_R1_ctl_npo")

        # C
        self.delParCns("temp_%s_3_jnt" % upr_pref, "echo_MouthUpperMid_C0_ctl_npo")
        self.delParCns("temp_%s_3_jnt" % lwr_pref, "echo_MouthLowerMid_C0_ctl_npo")
