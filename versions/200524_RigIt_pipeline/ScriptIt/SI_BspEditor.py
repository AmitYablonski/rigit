...
class BspEditor:
    ...
    def scriptIt(self, *args):
        ...
        if op is 1:  # add to existing
            ...
            if not bspName:
                ...
                return
            script += 'bShape = pm.PyNode("%s")\n' % bspName
            script += '\n' + self.getFindBspPos() + '\n'
            # add bShapes command
            script += "# add to blend shape\n"
            if bspName.partition(",")[1] == ",":
                script += "for bsp in bShapeGeo:\n"
                script += "\tpos = bspFreeIndex(bShape)\n"
                script += "\tpm.blendShape(bShape, edit=True, t=[geometry, pos, bsp, 1.0])\n"
            else:
                script += "pos = bspFreeIndex(bShape)  # position to add to the bsp node\n"
                script += "pm.blendShape(bShape, edit=True, t=[geometry, pos, bShapeGeo, 1.0])\n"
            ...
            script += ('# Create new blendShape\n'
                       'bShape = pm.blendShape(bShapeGeo, geometry, foc=True, n="bsp_name")[0]\n')
        if turnOn:
    ...
    def getFindBspPos(self):
        script = ''
        script += '# find the first available index in bsp\n'
        script += ('def bspFreeIndex(bsp):\n'
