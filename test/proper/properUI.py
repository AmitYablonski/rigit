...
class ProperUI(QtWidgets.QDialog):
    def __init__(self):
        # todo: singleton
        parent = getMayaMainWindow()
        super(ProperUI, self).__init__(parent=parent)
        self.setStyleSheet(sb_ui.load_stylesheets(["main_window"]))

        self.setWindowTitle("Proper")
        self.properInst = proper.Proper()
    ...
    def propItFunc(self):
        ...
        # Run proper
        self.properInst.run(selection=objects, tricycles=tricycles, align_tricycles=align_tricycles, dress_tricycles=dress_tricycles, mode_tricycles=mode_tricycles, helmet=helmet)

        # print log
        message = 'Props added successfully'
        if self.properInst.errorLog:
            message = "ERROR:\n"+"\n".join(self.properInst.errorLog)
        elif self.properInst.warningLog:
            message = "WARNING:\n"+"\n".join(self.properInst.warningLog)
        title = "LOG"
