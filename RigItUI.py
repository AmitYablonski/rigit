...
class RigItUI:
    ...
    def __init__(self, path=''):
        ...
        self.importFolders = ["RigItUI", "ScriptIt", "Guider"]
        self.rigItImports()
    ...
    def mayaToolsPopulate(self):
        ...
        self.shadingTools(parent)
        self.handySel(parent)
    ...
    def mainLayoutSetup(self):
        ...
        cmds.iconTextButton(style='iconOnly', image1=self.iconPath + "rigIt_top_210X33_001.jpg",
                            c='cmds.launch(web="https://www.youtube.com/watch?v=uyUXoap67N8")')
        ...
        self.widgets["Maya_Tools_frameLayout"] = cmds.frameLayout(label="Maya Tools", collapsable=True,
                                                                  bgc=self.mainFrames, w=self.mainWidth - 6,
                                                                  parent=self.widgets["mainLayout2"])
        self.widgets["MBA_Tools_frameLayout"] = cmds.frameLayout(label="Extra Tools", collapsable=True,
                                                                 bgc=self.mainFrames, w=self.mainWidth - 6,
                                                                 parent=self.widgets["mainLayout2"])
        ...
        cmds.iconTextButton(style='iconOnly', h=120, w=120, image1=self.iconPath + "rigIT_logo_002.jpg",
                            ...
                            c='cmds.launch(web="http://www.quickmeme.com/img/fd/fd38ba73d66041b5a6a9d6312bd99bc919a591bbf0b3ec90608ce92b4fb0e8fa.jpg")')
    ...
    def handyRiggingTools(self, parent='None'):
        ...
        cmds.button(l='Bind and\nCopy Skin', w=self.buttonW3, h=self.buttonH, c=gmt.bindAndCopy, bgc=[0.2, 0.6, .2],
                    ann='select source, then target/s.\nbind to source influences and copy weights')
        ...
        cmds.button(l='Transfer vtx\nWeights', w=self.buttonW3, c=gmt.transferWeights, bgc=[.6, .5, .8],
                    ann="Transfers skin weights from the first vertex to the others in the selection")
        cmds.button(l='Copy\nWeights', c=gmt.copyWeights, bgc=[.6, .5, .8],
                    ann="Copies skin weights from the first object to the others in the selection")
        cmds.button(l='Select skin\nInfluences', w=self.buttonW3, c=gmt.selectInflu, bgc=[.37, .48, .7],
                    ann='select all skins for selection')
        ...
        cmds.button(l='Material\nCopy', c=gmt.copy_material, bgc=[.5, .2, .2],
                    ann="Select source, then target/s to copy material/s")
    ...
    def cleanupTools(self, parent="None"):
        ...
        cmds.button(l='LRC safe\nbaker', w=self.buttonW3, bgc=bColor2, h=self.buttonH + 1,
                    c=self.openSafeBaker, ann="A tool to debug lrc shots")
    ...
    def handySel(self, parent="None"):
        if parent == 'None':
            return "Can't load handySel"
        bColor = [.5, .6, .4]
        bColor2 = [.46, .6, .37]
        self.widgets["handySel_frameLayout"] = cmds.frameLayout(label="Handy Selections", collapsable=True,
                                                                bgc=self.secFrames, p=parent)
        self.secFramesLayouts["handySel_frameLayout"] = self.widgets["handySel_frameLayout"]
        self.widgets["handySel_Layout"] = cmds.rowColumnLayout(numberOfColumns=3, bgc=self.buttonColor2,
                                                               parent=self.widgets["handySel_frameLayout"])
        self.set2rowColumnLayouts["handySel_Layout"] = self.widgets["handySel_Layout"]
        cmds.button(l='local\nconstraints', w=self.buttonW3, h=self.buttonH, bgc=bColor,
                    c=partial(gms.selLocalCns, True), ann="Select all none-referenced constraints in scene")
        cmds.button(l='all\nconstraints', w=self.buttonW3, h=self.buttonH, bgc=bColor2,
                    c=partial(gms.selLocalCns), ann="Select all constraints in scene")
    ...
    def funkyTools(self, parent="None"):
        ...
        cmds.iconTextButton(style='iconAndTextVertical', label='Guider',
                            w=self.buttonW4, c=self.openGuiderUI, image1=self.iconPath + 'guiderIcon.png',
                            ann="Opens Guider - mGear toolbox")
        ...
        cmds.iconTextButton(style='iconAndTextVertical', label='ColorIt', image1="colorProfile.png",
                            w=self.buttonW4, ann="Opens Color Switcher creator window",
                            c=self.openColorIt)
        cmds.iconTextButton(style='iconAndTextVertical', label='Duplicator', image1="duplicateReference.png",
                            w=self.buttonW4, ann="Opens Duplicator window",
                            c=self.openDuplicator)
        scriptsPath = cmds.internalVar(usd=True)
        cmds.iconTextButton(style='iconAndTextVertical', label='Material\nCopier', image1="colorProfile.png",
                            w=self.buttonW4, ann="Opens Material Copier window",
                            c=self.openMaterialCopier)
        cmds.iconTextButton(style='iconAndTextVertical', label='Test', image1="question.png",
                            w=self.buttonW4, ann="Test",
                            c=self.openTest)

    def shadingTools(self, parent="None"):
        if parent == 'None':
            return "Can't load shadingTools"
        # todo add material copy tool (add option to copy from source and target lists)
        # todo assignMtl
        '''
        i've added "assignMtl" script to Rigit
        [P:\MBA_SE02\scripts\rigging\RigIt\collectiveTools\mayaTools]
        
        as of now all it does is identify the material on the first selected object and assign it to the second selected object
        
        issues and todos : 
        issue 01 - if there is more then one material on the source object the script won't work
        issue 02 - if there is more/less then 2 objects selected the tool will fail
        and many more issue i just don't know about right now
        
        todo 01 - make an option to select what mtl you want to move from selected obj
        todo 02 - make an option to assign a mtl from 1 obj to many other obj
        todo 03 - if both objects are copies of one another then move every mtl and assign per faces 
        todo 04 - assign objects from lists (like in UV transfer master)
        and anything else you think we can do with this tool or maybe ways it will help us or shading with our day to day wor
        '''
        # handyPrints_frameLayout
        self.widgets["shadingTools_frameLayout"] = cmds.frameLayout(label="Shading Tools", collapsable=True,
                                                                    bgc=self.secFrames, p=parent)
        self.secFramesLayouts["shadingTools_frameLayout"] = self.widgets["shadingTools_frameLayout"]
        self.widgets["shadingTools_Layout"] = cmds.rowColumnLayout(numberOfColumns=4, bgc=self.buttonColor1,
                                                                   parent=self.widgets["shadingTools_frameLayout"])
        self.set1rowColumnLayouts["shadingTools_Layout"] = self.widgets["shadingTools_Layout"]
        cmds.iconTextButton(style='iconAndTextVertical', label='UV Trans\nMaster',
                            image1=self.iconPath + "UVtoolKit.png",
                            w=self.buttonW4, ann="",
                            c=self.openUVtransMaster)
        cmds.iconTextButton(style='iconAndTextVertical', label='VRay\nID Tool', image1=self.iconPath + "question.png",
                            w=self.buttonW4, ann="", h=40,
                            c=self.openVRay_ID_Tool)
        cmds.iconTextButton(style='iconAndTextVertical', label='Mult\nUV Link', image1=self.iconPath + "UVtoolKit.png",
                            w=self.buttonW4, ann="Connect multiple objects map",
                            c=self.openMultiUVConnect)

    def snowballToolsFrame(self, parent='None'):
        if parent == 'None':
            return "Can't load snowballToolsFrame"
        self.widgets["snowTools_frameLayout"] = cmds.frameLayout(label="Studio_Tools", collapsable=True,
                                                                 bgc=self.secFrames, p=parent)
        ...
        cmds.iconTextButton(style='iconAndTextVertical', image1=self.iconPath + "FinRig_icon_02.png",
                            ...
                            w=self.buttonW4, h=h, c=self.openRigFinalizer)

        scriptsPath = cmds.internalVar(usd=True)
        if 'amirr' in scriptsPath:
            cmds.iconTextButton(style='iconAndTextVertical', image1=self.iconPath + "FinRig_icon_02.png",
                                ann="Finalize rig for props - make cns, check hierarchy and enable override.",
                                w=self.buttonW4, h=h, c=self.openRigFinalizer_qt)
    ...
    def openPipeline(self, *args):
        from pipeline.variants import pipeline_maya
        pipeline_maya.start()
    ...
    ''' # example:
    def toolLauncher(tool, path='', scriptIt=False, *args):
        import tool
        reload(tool)
        tool.tool()
    '''
    ...
    @staticmethod
    def openRigFinalizer_qt(*args):
        import RigFinalizer_qt
        reload(RigFinalizer_qt)
        RigFinalizer_qt.start_app()

    @staticmethod
    def openRigFinalizer(*args):
        import RigFinalizer
        reload(RigFinalizer)
        RigFinalizer.RigFinalizer()  # scriptIt=True)
    ...
    def openLiteWrap(*args):
        import LiteWrap
        reload(LiteWrap)
        LiteWrap.LiteWrap(scriptIt=True)
    ...
    @staticmethod
    def openColorIt(*args):
        import ColorIt
        reload(ColorIt)
        ColorIt.ColorIt(scriptIt=True)

    @staticmethod
    def openMaterialCopier(*args):
        import MaterialCopier
        reload(MaterialCopier)
        MaterialCopier.MaterialCopier(scriptIt=True)

    @staticmethod
    def openDuplicator(*args):
        import Duplicator
        import Duplicator
        reload(Duplicator)
        Duplicator.Duplicator(scriptIt=True)

    @staticmethod
    def openTest(*args):
        from pipeline.variants.pipeline_maya.utils import maya_qt
        import RigItBasicWindow_qt
        reload(RigItBasicWindow_qt)
        RigItBasicWindow_qt.start_app()
        # maya_qt.show(RigItBasicWindow_qt)
    ...
    @staticmethod
    def openSafeBaker(*args):
        import SafeBaker
        reload(SafeBaker)
        SafeBaker.SafeBaker()

    @staticmethod
    def openVRay_ID_Tool(*args):
        import VRay_ID_Tool
        reload(VRay_ID_Tool)
        VRay_ID_Tool.VRay_ID_Tool(scriptIt=True)

    @staticmethod
    def openMultiUVConnect(*args):
        import multiUVConnect
        reload(multiUVConnect)
        multiUVConnect.multiUVConnect()
